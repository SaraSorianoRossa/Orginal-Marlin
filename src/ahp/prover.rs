...
impl<F: PrimeField> AHPForR1CS<F> {
    ...
    pub fn prover_third_round<'a, R: RngCore>(
        ...
        // the prover has alpha and all the etas from the verifier        
        let VerifierFirstMsg {
            eta_a,
            eta_b,
            eta_c,
        ...
        // the prover receives beta = beta_1
        ...
        let f = EvaluationsOnDomain::from_vec_and_domain(f_vals_on_K, domain_k).interpolate(); // sumcheck over K
        end_timer!(f_poly_time);

        let g_2: DensePolynomial<F> = DensePolynomial::from_coefficients_slice(&f.coeffs[1..]);
        ...
        // Computing h_2 = (a(X) - b(X) (X*g_2(X)+t(beta)/domain(K)))/vanishing_K
        ...
        // prover sends g_2 and h_2 to verifier
